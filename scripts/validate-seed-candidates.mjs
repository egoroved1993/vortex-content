...
export function scoreCandidate(candidate, index = 0, cityAnchorsLower = cityAnchors, randFn = () => 0.5) {
  ...
  const hasCyrillicImplicitFirstPerson =
    /(^|[^\p{L}\p{N}_])(заш[её]л|вышел|купил|взял|сел|сидел|стоял|ждал|пош[её]л|подумал|понял|решил|поймал себя|сказал себе|оправдываю|делаю вид)(?=$|[^\p{L}\p{N}_])/iu.test(content);
  ...
  const signals = {
    firstPerson:
      ...
      /\b(he estado|he visto|he ido|estoy|tengo|vivo|odio|tinc|porto|vaig|visc|m'agrada|m’agrada)\b/i.test(content) ||
    ...
    implicitFirstPerson:
      ...
      /^[\s"'“”]*(?:esta mañana|hoy|avui|ahir|ayer)[,\s]+(?:he|vaig|porto|tinc|estoy|tengo|me|em)\b/i.test(content) ||
      hasCyrillicImplicitFirstPerson,
    dialogue: /[“’””«»]/.test(content) || /\bsaid\b/i.test(content),
    detail: /(\d|€|\$|£|:|line \d|line \w|\bl\d\b|stop|platform|queue|rent|alquiler|lloguer|piso|pis|vivienda|habitatge|coffee|espresso|cortado|flat white|tram|bus|train|tren|metro|rodalies|fgc|tmb|ube?r?bahn|tube|bart|muni|sp[aä]ti|pub|barista|landlord|roommate|bodega|fog|startup|kebab|canal|market|bakery|corner shop|overground|victoria line|u8|ringbahn|metro line|dolores|mission|sunset district|painted ladies|brick lane|pret|hackney|peckham|islington|dalston|gracia|gràcia|raval|barceloneta|eixample|poblenou|sants|superblock|neukolln|prenzlauer|kreuzberg|friedrichshain|spati|maletas|maletes|barrio|barri|calle|carrer|plaça|plaza|terraza|terrassa|lavabo|baño|bany|parque|parc|polen|plataneros|plataners|vecino|vecina|veí|veïns|veins|ticket|tickets|gig|concert|festival|screening|lineup|venue|door|guestlist|club|exhibition|каталан|каталан|барселон|барс[аеу]|шум|miete|l3|очеред|дождь|туман|кофе|автобус|метро|турист|чемодан|кухн|холодильник|официант|сосед|велосипед|граффити|двор|бабушка|местные|аренд|хозяин|еда|толпа|деньги|вермут|кава|вино|джаз|абсент|коктейл|сэндвич|креветк|тортиль|какаолат|бомб[ао]й)/i.test(content),
    anchor:
      ...
      /(барселон|барс[аеу]|берлин|лондон|сан[- ]?франц|san francisco|barcelona|berlin|london)/i.test(content),
    hook: /(still|again|weirdly|somehow|for some reason|caught myself|keep|pretend|told myself|cannot stop|can't stop|why does|i hate|i love|never gets old|already regretting|forgot|left .* at home|lost (my|their|his|her|the)|otra mañana|madre de dios|c[aá]gate|cada vez|me hace gracia|todavía|encara|sempre|siempre|cada cop|em fa gr[aà]cia|quina mandra|ja podem plegar|no hay manera|no hi ha manera|смешно|все равно|всё равно|до сих пор|каждый раз|каждое утро|каждый день|понимаешь|не знаю|никто|вдруг|поймал себя|сказал себе|оправдываю|быстро зайду|не совсем провален|wieder|immer noch)/i.test(content),
    pettySpecificity:
    ...
    conflict: /(argued|fighting|annoying|delay|late|awkward|rent|alquiler|lloguer|expensive|caro|cara|car[ií]ssim|carisimo|shame|embarrass|wrong|mad|tired|replaced|gone|disappeared|lost|forgot|overpriced|changed|can't afford|pushed out|no longer|used to be|spill|wet socks|broken zipper|turístic|turistico|turistas|turistes|guiri|maletas|maletes|ruido|soroll|harto|fart|merda|mierda|asco|locura|ll[aà]stima|lastima|imposible|fraude|fraudulent[oa]|estafa|estafaron|tancat|cerrado|saturad|colaps|retard|vaga|avaria|averia|teure|teuer|chaos|задерж|шум|дорого|турист|уволили|продал|спорят|деньги|жду|ждать|очеред|чемодан|не помогает)/i.test(content),
...
function normalizeCandidateLanguage(value, content = "") {
  ...
  if (raw === "ca" || raw === "catalan" || raw === "català" || raw === "catala") return "ca";
...
function hasCityLanguageMismatch(candidate, detectedLanguage, content) {
  if (candidate.sourceFamily !== "event_discovery") return false;
  ...
  if (candidate.cityId === "berlin") {
    return normalized && !["de", "en", "ru"].includes(normalized);
  }
  return false;
}
