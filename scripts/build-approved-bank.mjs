...
const HEADLINE_OR_SEO_RE = /watch the latest .* forecast|\bhouses for rent in [A-Z]|\bSo teuer ist Wohnen\b|\bNeues Quartier entsteht\b|\bsummerlike weather forecast\b|\bBay Area weather shifts from wet to warm\b|\bITV weather forecast\b|\bDiscovering hidden London\b|\bRead more\b|\bSubscribe now\b/i;
...
const NOSTALGIA_SLOP_RE = /\b(год назад|тогда .*теперь|ощущение то же самое|first time in my life|i used to spend a lot of time|used to be .* now|i remember seeing|thought about it over the years|20 years since|since the early 2000s|sadly it[’']s closed now|l'any passat|el año pasado|fa anys|hace años|abans .* ara|antes .* ahora)\b/i;
